...
///     <para>
///         Перегрузки <c>Signal</c> рисуют полупрозрачные вертикальные полосы.
///         Благодаря полупрозрачности можно рисовать несколько <c>Signal</c> и цвета будут смешиваться для придания
///         дополнительной семантики.
///     </para>
///     <para>Перегрузки <c>Level</c> рисуют горизонтальные прямые.</para>
///     <para>
///         Некоторые перегрузки через модификатор out возвращают цвет, который был использован для рисования, чтобы им
///         можно было продолжить рисовать что-то ещё.
///     </para>
///     <para>
///         Если цвет не указан, то из палитры автоматически выбирается уникальный цвет. По умолчанию используется
///         нейтральная палитра <see cref="AnimalColor.Neutral" />.
///     </para>
/// </summary>
public interface IPaint : ICharter
{
    /// <summary>
    ///     Рисует свечи.
    /// </summary>
    ...
    /// <summary>
    ///     Рисует одну или несколько невидимых горизонтальных границ, чтобы видимые границы оси ординат оставались на своих
    ///     местах во время прокрутки.
    /// </summary>
